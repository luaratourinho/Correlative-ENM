...
envi.mask <- mask(envi.cut,mcp_buffer)

# Saving rasters
dir.create(paste0("./data/env_croped/", "."))
writeRaster(envi.mask, filename='./data/env_croped/', format="GTiff", 
...
future_var_stk <- stack("./data/env/wc2.1_10m_bioc_BCC-CSM2-MR_ssp585_2041-2060.tif")
...
#IPSL
future_var_stk <- stack("./data/wc2.1_2.5m_bioc_IPSL-CM6A-LR_ssp585_2041-2060.tif")
envi_fut.cut<- crop(future_var_stk, to_crop)
envi_fut.mask<- mask(envi_fut.cut, to_crop)
dir.create(paste0("./data/env_cropped/future/ip50"))
writeRaster(envi_fut.mask, filename='./data/env_cropped/future/ip50/', format="GTiff", 
            bylayer=TRUE, suffix="names", overwrite=TRUE)

#MIROC6
future_var_stk <- stack("./data/wc2.1_2.5m_bioc_MIROC6_ssp585_2041-2060.tif")
envi_fut.cut<- crop(future_var_stk, to_crop)
envi_fut.mask<- mask(envi_fut.cut, to_crop)
dir.create(paste0("./data/env_cropped/future/mi50"))
writeRaster(envi_fut.mask, filename='./data/env_cropped/future/mi50/', format="GTiff", 
            bylayer=TRUE, suffix="names", overwrite=TRUE)
